...
class Step_1(Scene):
    def construct(self):
        ...
        circle = Circle(radius=0.2, color=BLACK, stroke_width=3).move_to(axes.c2p(3, 0)).shift(DOWN*0.15+LEFT*0.08)
        #curve
        f = lambda x: 1/(x-3)
        ...
        dot1 = Dot(axes.c2p(x_right_limit, y_top), color="#1F2A7C", radius=0.06).shift(LEFT*0.08+UP*0.03)
        dot2 = Dot(axes.c2p(x_left_limit, y_bottom), color="#1F2A7C", radius=0.06).shift(DOWN*0.02+LEFT*0.05)
