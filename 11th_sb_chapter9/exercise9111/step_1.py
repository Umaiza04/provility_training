...
class Step_1(Scene):
    def construct(self):
        ...
        y_label1 = axes.get_x_axis_label(MathTex("y'", color=BLACK)).shift(DOWN*4.8+LEFT*5.6)
        x_label1 = axes.get_y_axis_label(MathTex("x'", color=BLACK)).shift(DOWN*4.0+LEFT*5.6)

        three_minus_arrow = Arrow(axes.c2p(-3, 0.5), axes.c2p(2.5, 0.5), color="#A87900", stroke_width=3, tip_length=0.15).shift(UP*0.2+RIGHT*0.4)
        three_minus_text = MathTex("3^-", color=BLACK, font_size=34).next_to(three_minus_arrow, UP, buff=0.1).shift(RIGHT*1.3)

        three_plus_arrow = Arrow(axes.c2p(5.2, 0.5), axes.c2p(3.5, 0.5), color="#A87900", stroke_width=3, tip_length=0.15).shift(UP*0.2+LEFT*0.6)
        three_plus_text = MathTex("3^+", color=BLACK, font_size=34).next_to(three_plus_arrow, UP, buff=0.1).shift(LEFT*0.15)
        ...
        x_left_limit = 3.005+ 1/y_bottom
        #left curve
        left_curve = axes.plot(f,x_range=[-4.5, x_left_limit],color="#1F2A7C",stroke_width=4,use_smoothing=False).shift(UP*0.15+LEFT*0.04)
        ...
        dot1 = Dot(axes.c2p(x_right_limit, y_top), color="#9D0216", radius=0.06).shift(LEFT*0.06+UP*0.03)
        dot2 = Dot(axes.c2p(x_left_limit, y_bottom), color="#9D0216", radius=0.06).shift(DOWN*0.02+LEFT*0.05)
        ...
        label3 = Text("At x = 3", color=BLACK, font_size=25).rotate(90*DEGREES).next_to(line, LEFT, buff=0.1).shift(UP*2)
        line2 = Line(start=axes.c2p(-1, -1),end=axes.c2p(1, -1), color="#B96902", stroke_width=4).shift(UP*0.2)
        dot3 = Dot(line2.get_start(), color="#6832E3", radius=0.05)
        dot4 = Dot(line2.get_end(), color="#6832E3", radius=0.05)

        self.add( axes, x_label, y_label, x_label1,y_label1, left_curve, right_curve,line,label1,label2,up_arrow,down_arrow,up_text,down_text,
                 three_minus_arrow,three_minus_text,three_plus_arrow,three_plus_text,circle,dot1, label3,dot2,line2,dot3,dot4,)
