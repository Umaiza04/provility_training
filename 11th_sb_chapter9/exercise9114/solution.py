...
class Step_1(Scene):
    ...
    axis_config={
        ...
        x_label1 = axes.get_x_axis_label(MathTex("x'", color=BLACK)).shift(DOWN*4.3+LEFT*5.2)
        y_label1 = axes.get_y_axis_label(MathTex("y'", color=BLACK)).shift(DOWN*6.5+LEFT*5.6)
        ...
        func_label = MathTex(r"f(0) = sec",color=BLACK).next_to(axes.c2p(PI/2, 5.5), UP, buff=0).shift(RIGHT*1.5)
        zero_minus_tex = MathTex("0^-",color=BLACK).move_to(axes.c2p(-1.1, 0.7))
        zero_plus_tex = MathTex("0^+", color=BLACK).move_to(axes.c2p(1.1, 0.7))
        arrow_left = Arrow(
             start=axes.c2p(-1.5, 0.4),
             end=axes.c2p(-0.2, 0.4),color="#A87900",stroke_width=3,)
        arrow_right = Arrow(
            start=axes.c2p(1.5, 0.4),
            end=axes.c2p(0.2, 0.4),color="#A87900",stroke_width=3,)
